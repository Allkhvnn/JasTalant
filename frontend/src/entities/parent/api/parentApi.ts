...
import type { DevelopmentAssessment } from '../../development/model/types'
...
export function getChildDevelopment(
  token: string,
  academyId: string,
  playerId: string,
  page = 0,
  ...
  const query = new URLSearchParams({ page: String(page), size: String(size) })
  return apiRequest<PageResponse<DevelopmentAssessment>>(
    `/api/academies/${academyId}/parent/players/${playerId}/development-assessments?${query}`,
    { token },
  )
}
