...
reverse proxy; прикладное ограничение сейчас есть только у повторной отправки кода.
...
JWT действует 15 минут; refresh-токенов пока нет, после истечения нужен повторный вход.
...
Ответ содержит `accessToken`, `tokenType=Bearer`, `expiresIn=900`.
...
Сейчас это backend API, формы frontend не создавались. SMTP в HTTP-тестах
...
RLS, восстановление пароля,
refresh-токены, уведомление об одобрении по email и повторная подача заявки
